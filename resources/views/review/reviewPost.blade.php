...
@section('container')
    ...
    <div class="row mb-3">
        <div class="row justify-content-center mb-5">
            <div id="containerPost" class="col-md-10">
                ...
                <h1 class="text-center">{{ $reviewPost->title }}</h1>
                
                <div id="infoCont" class="my-4">
                    <h4>
                        <div>
                            <span class="info">Author</span> 
                            <span>: {{ $reviewPost->user->name }}</span> 
                        </div>
                        
                        <div>
                            <span class="info">Category</span>
                            <span>: {{  $reviewPost->category->name }}</span> 
                        </div>
                        
                        <div>
                            <span class="info">Publish Date</span>
                            <span>: {{ $reviewPost->created_at->format('j F Y') }}</span>
                        </div>
                        <div id="like">
                            {{-- <form id="addLike" method="post">
                                @csrf
                                <input id="reviewIDLike" type="hidden" name="reviewID" value="{{ $reviewPost->id }}">
                ...
                <div class="mb-2" style="">
                    @auth
                        @if ($reviewPost->user_id == auth()->user()->id)
                            <a href="/reviewPosts/{{ $reviewPost->slug }}/edit" class="btn font-weight-bold m-0 ml-1" style="background-color: #8c52ff; color: #fffccf !important; text-decoration: none; font-size: 15px">Edit</a>
                            <form action="/reviewPosts/{{ $reviewPost->slug }}" method="post" class="font-weight-bold bg-dark d-inline m-0" style="height:100px">
                                @method('delete')
                                @csrf
                ...
                @if ($reviewPost->image)
                    <img id="contentimg" src="{{ asset('storage/' . $reviewPost->image) }}" alt="{{ $reviewPost->category->name }}" class="img-fluid ">
                @else
                    <img id="contentimg" src="https://source.unsplash.com/1200x400?{{ $reviewPost->category->name }}" alt="{{ $reviewPost->category->name }}" class="img-fluid ">
                @endif
                        ...
                        <div class="col-sm-12">
                            <div class="comment-wrapper">
                                <div class="panel panel-info">
                                    ...
                                    {{-- action="/reviewPosts/{{ $reviewPost->slug }}/storeComment" --}}
                                    <div class="mt-2">
                                        <form id="addComment" 
                                        {{-- action = "/reviewPosts/{{ $reviewPost->slug }}/storeComment"  --}}
                                        method="post">
                                            @csrf
                                            <input id="reviewID" type="hidden" name="reviewID" value="{{ $reviewPost->id }}">
                                            <input id="reviewSlug" type="hidden" name="reviewSlug" value="{{ $reviewPost->slug }}">
                                            @auth
                                                <div id="textarea">
    ...
    <script>
        var id = parseInt(document.getElementById('reviewID').value.toString());
        var slug = document.getElementById('reviewSlug').value.toString();
        ...
        function getAllLike() {
            $.get("{{ url('/reviewPosts/${slug}/getAllLike') }}", {reviewID: id}, function(response, status) {
                ...
                @auth
                    for(var i = 0; i < response.length; i++) {
                        if(response[i].user_id.toString() == authUserID) {
                            ...
                            `<input id="reviewIDLike" type="hidden" name="reviewID" value="${id}">
        ...
        function storeLike(IDuser, IDreview) {
            event.preventDefault();
            $.ajax({
                url: '/reviewPosts/' + slug + '/storeLike',
                type: 'post',
                data: {userID : IDuser, reviewID : IDreview, _token : '{{csrf_token()}}'},
                dataType: 'json',
        ...
        function getAllComment() {
            $.get("{{ url('/reviewPosts/${slug}/getAllComment') }}", {reviewID: id}, function(response, status) {
                console.log(response);
        ...
        $('form#addComment').submit(function( event ) {
            ...
            if(check.length!=0) {
                $.ajax({
                    url: '/reviewPosts/' + slug + '/storeComment',
                    type: 'post',
        ...
        function deleteComment(deleteID) {
            $.ajax({
                url: '/reviewPosts/' + slug + '/destroyComment',
                type: 'post',
        ...
        function deleteLike(likeID) {
            $.ajax({
                url: '/reviewPosts/' + slug + '/destroyLike',
                type: 'post',
        ...
        function updateComment(editID, commentID, action) {
            ...
            if(checkEdit.length!=0) {
                $.ajax({
                    url: '/reviewPosts/' + slug + '/updateComment',
                    type: 'post',
