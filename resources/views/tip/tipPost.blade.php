...
@section('container')
    ...
    <div class="row mb-3">
        <div class="row justify-content-center mb-5">
            <div id="containerPost" class="col-md-10">
                ...
                <div class="mb-2" style="">
                    @auth
                        @if ($tipPost->user_id == auth()->user()->id)
                            <a href="/tipPosts/{{ $tipPost->slug }}/edit" class="btn font-weight-bold m-0 ml-1" style="background-color: #8c52ff; color: #fffccf !important; text-decoration: none; font-size: 15px">Edit</a>
                            <form action="/tipPosts/{{ $tipPost->slug }}" method="post" class="font-weight-bold bg-dark d-inline m-0" style="height:100px">
                                @method('delete')
                                @csrf
                ...
                @if ($tipPost->image)
                    <img id="contentimg" src="{{ asset('storage/' . $tipPost->image) }}" alt="{{ $tipPost->category->name }}" class="img-fluid ">
                @else
                            ...
                            <div class="comment-wrapper">
                                <div class="panel panel-info">
                                    ...
                                    <div class="mt-2">
                                        ...
                                        method="post">
                                            @csrf
                                            <input id="tipID" type="hidden" name="tipID" value="{{ $tipPost->id }}">
                                            <input id="tipSlug" type="hidden" name="tipSlug" value="{{ $tipPost->slug }}">
                                            @auth
                                                <div id="textarea">
    ...
    <script>
        var id = parseInt(document.getElementById('tipID').value.toString());
        var slug = document.getElementById('tipSlug').value.toString();
        ...
        function getAllLike() {
            $.get("{{ url('/tipPosts/${slug}/getAllLike') }}", {tipID: id}, function(response, status) {
                ...
                @auth
                    for(var i = 0; i < response.length; i++) {
                        if(response[i].user_id.toString() == authUserID) {
                            ...
                            `<input id="tipIDLike" type="hidden" name="tipID" value="${id}">
        ...
        function storeLike(IDuser, IDtip) {
            event.preventDefault();
            $.ajax({
                url: '/tipPosts/' + slug + '/storeLike',
                type: 'post',
                data: {userID : IDuser, tipID : IDtip, _token : '{{csrf_token()}}'},
                dataType: 'json',
        ...
        function getAllComment() {
            $.get("{{ url('/tipPosts/${slug}/getAllComment') }}", {tipID: id}, function(response, status) {
                console.log(response);
        ...
        $('form#addComment').submit(function( event ) {
            ...
            if(check.length!=0) {
                $.ajax({
                    url: '/tipPosts/' + slug + '/storeComment',
                    type: 'post',
        ...
        function deleteComment(deleteID) {
            $.ajax({
                url: '/tipPosts/' + slug + '/destroyComment',
                type: 'post',
        ...
        function updateComment(editID, commentID, action) {
            ...
            if(checkEdit.length!=0) {
                $.ajax({
                    url: '/tipPosts/' + slug + '/updateComment',
                    type: 'post',
